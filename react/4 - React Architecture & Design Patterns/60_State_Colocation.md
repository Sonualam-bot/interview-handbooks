...
### Why Colocation's Performance Story Is Real But Easy to Overstate

Section 4's caveat — don't reduce this to "higher state always means
worse performance" — deserves the actual mechanism behind it. When
`App`'s `name` state changes and `Header` sits beside `Profile` as a
sibling, React still has to reconcile `Header`'s subtree: walk into
it, compare its previous React Elements against the newly-created
ones, and confirm nothing changed — even though the DOM never ends up
touched (see Handbook 1/2, Reconciliation). That comparison work is
real and scales with the size of `Header`'s subtree, not with whether
anything visually changes. For a small `Header`, this cost is
negligible — colocating `name` inside `Profile` buys essentially
nothing measurable. For a `Header` containing hundreds of nodes or
expensive child computations, the unnecessary reconciliation walk is
the actual, measurable cost — and colocating the state (or, per
Chapter 51, passing `Header` down as `children` instead of a sibling)
removes it structurally, since `App`'s re-render would then never
recreate `Header`'s element at all.

### Colocation's Real Justification Is Blast Radius, Not Speed

The stronger, version-independent argument for colocation is about
**what a future change can affect**, not runtime cost. State owned by
`App` is state that any code reading or modifying `App` might need to
reason about — a bug in `Profile`'s input handling could, in
principle, be caused by something happening in `App`, simply because
that's where the state lives. State owned by `Profile` itself has a
blast radius of exactly one component: nothing outside `Profile` can
read it, set it, or introduce a bug in how it changes. This is why
colocation remains good practice even in the (common) case where
reconciliation cost is genuinely irrelevant — it's fundamentally a
change-isolation principle, and the re-render angle is a secondary,
situational bonus rather than the core reason for the rule.
