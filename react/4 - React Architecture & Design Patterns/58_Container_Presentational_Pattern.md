...
### Why Presentational Components Are Actually Easier to Test — Precisely

Section 9 says presentational components are "easier to test" — the
precise reason is that a pure presentational component is
**referentially transparent**: given the same props, it always
produces the same output, with no dependency on timing, network
responses, or side effects. Testing it means calling it with props and
asserting on the output — no mocking `fetch`, no waiting for a promise
to resolve, no simulating async timing.

A container, by contrast, is fundamentally about *when* things happen
— when the effect fires, when the fetch resolves, when loading flips
to `false`. Testing it can't be reduced to "input → output" in the
same way, because its correctness is about a sequence of state
transitions over time, not a single deterministic mapping. This is why
splitting the two apart doesn't just organize code more clearly — it
genuinely changes *what kind* of test each piece needs: the
presentational half gets fast, deterministic unit tests; the container
half still needs integration-style tests that deal with async timing,
regardless of how the split is implemented (a wrapping component or a
custom Hook).

### Why the Fetch Itself Was Never the Actual Problem

It's worth being precise about what "separating concerns" bought here.
`fetch("/api/user")` isn't inherently harder to reason about than
`<h1>{user.name}</h1>` — both are simple in isolation. The actual
problem the original mixed component had was that a single function
was responsible for two things that change **for different reasons**
and **on different timelines**: the data-fetching logic changes when
the API changes; the markup changes when the design changes. Container
splitting (or, later, extracting a custom Hook — see Section 11)
doesn't make either half simpler on its own — it ensures a change to
one reason doesn't require touching, re-reading, or re-testing code
that exists for the other reason.
