...
### Why "Shared Depends on Feature" Isn't Just Bad Style — It Risks a Real Circular Import

Section 13's rule (`components/Button` should never depend on
`features/cart`) has a concrete failure mode, not just a conceptual
one. If `Button` imports something from `features/cart`'s public API
(Section 11's `index.js`), and `cart`'s components import `Button`
from the shared layer, the two modules now import from each other —
directly or through their barrel files. JavaScript module resolution
handles circular imports by giving each module a partially-initialized
version of the other at the moment of the cycle, which depending on
evaluation order can mean one side sees `undefined` for a binding that
genuinely exists, just not yet assigned. This typically doesn't throw
immediately — it surfaces as an intermittent "cannot read property of
undefined" that depends on which module happens to load first,
exactly the kind of bug that's hard to reproduce and easy to
misdiagnose as unrelated. Keeping dependencies one-directional
(feature → shared, never the reverse) isn't just conceptually cleaner
— it's what makes this entire class of bug structurally impossible.

### The Real Cost of the Public-API Barrel File: Tree-Shaking

Section 11's `index.js` re-export pattern buys encapsulation, but it
has a genuine trade-off worth naming: a barrel file that re-exports
many things from a feature can make it harder for a bundler to
tree-shake unused code, because importing *anything* from
`@/features/cart` may force the bundler to evaluate `index.js`'s full
module graph to be sure nothing else has side effects, even if the
consumer only needed `useCart`. This doesn't mean barrel files are
wrong — the encapsulation benefit is usually worth it — but it's the
concrete price behind "internal structure can change without
consumers changing," and it's why some codebases deliberately use more
targeted re-exports (or bundler-specific `sideEffects: false`
annotations in `package.json`) rather than one all-encompassing
barrel per feature.
