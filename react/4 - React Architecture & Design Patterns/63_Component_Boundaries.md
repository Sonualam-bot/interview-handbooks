...
### Why "Creating a Component Doesn't Automatically Make React Render Less" Is Literally True

Section 17 states this but it's worth being precise about why a
boundary, by itself, changes nothing. Extracting a piece of JSX into
`function Widget() { return ...; }` and calling `<Widget />` instead of
inlining the same markup produces the *same* React Elements, the same
reconciliation work, the same commit — a boundary is purely a source-
code-organization decision until something else acts on it. What
actually changes rendering behavior are separate mechanisms that a
boundary merely makes *available* to use:

- Wrapping the new component in `React.memo` (Handbook 3) — now there's
  a component-shaped unit React can shallow-compare props against and
  potentially bail out of re-rendering.
- Moving state inside the boundary (Chapter 60, State Colocation) — now
  a state change re-renders only that component's subtree, not
  whatever used to contain it.
- Passing the boundary down as `children` (Chapter 51) — now its
  parent's re-renders don't recreate its element at all.

A boundary is the *precondition* for each of these — you can't
`React.memo` a piece of inline JSX, and you can't colocate state
somewhere that isn't its own component — but drawing the boundary
alone applies none of them. This is exactly why "creating components
for performance" without following through with one of the above
techniques is a wasted boundary: it adds organizational overhead while
leaving the actual render behavior identical to before.
