...
### The Actual Trade-off Behind "Prop Drilling Is Not Always Bad"

Section 5 correctly resists "always avoid prop drilling," but the
precise reason explicit props are sometimes *preferable* to Context or
global state is traceability. A prop is visible at every step of its
journey — open `Dashboard`, and `user` is sitting right there as a
parameter, explicitly received and explicitly forwarded; "find
references" or a simple read-through of the file tells you everything
about where it came from and where it's going. A value read from
Context or an external store has none of that local visibility —
`useContext(UserContext)` inside `UserProfile` tells you nothing about
which Provider supplies it, how many other places also consume it, or
what triggers it to change, without separately locating the Provider
and every other consumer. This is the real cost side of "alternatives
to prop drilling": they trade *local traceability* for *convenience at
a distance* — genuinely worth it once drilling crosses several
unrelated layers (Section 3), but a real cost, not a free upgrade,
for the shallow, meaningful-at-every-step case Section 5 describes.
