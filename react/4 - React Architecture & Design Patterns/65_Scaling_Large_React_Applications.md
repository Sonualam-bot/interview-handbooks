...
### How `lazy` + `Suspense` Actually Works: Throwing a Promise

Section 16 shows the API without the mechanism, and it's a genuinely
unusual one. `import("./Settings")` returns a Promise that resolves
once the bundler fetches and evaluates the separate chunk containing
`Settings`. `React.lazy` wraps that Promise in a special component
type. When React tries to render it and the Promise hasn't resolved
yet, the wrapped component doesn't return JSX at all — it **throws the
pending Promise itself**, not an error. `Suspense` is a boundary that
specifically catches *thrown Promises* (as distinct from
`ErrorBoundary`, which catches thrown errors), and reacts by rendering
its `fallback` and re-subscribing to the thrown Promise. Once the
Promise resolves, React knows the previously-thrown work is now ready
and re-attempts the render — this time `Settings` is fully loaded and
renders normally. This "throw a Promise to signal not-ready-yet" trick
is the actual mechanism behind every use of `Suspense`, not just code
splitting — it's a generic "pause rendering until this async thing
resolves" protocol that data-fetching libraries also hook into.

### How Virtualization Renders 20 DOM Nodes But Scrolls Like 100,000

Section 18 says only the visible ~20 items reach the DOM — here's how
that doesn't just leave the scrollbar broken. A virtualized list
renders a container sized to the *total* height all 100,000 items
would occupy if they were all real (`itemCount * estimatedItemHeight`)
— an empty spacer, not actual content — so the browser's native
scrollbar behaves exactly as if the full list existed. As the user
scrolls, the library reads the current scroll offset, computes which
item indices *would* be visible at that offset, and renders only
those ~20 real items, absolutely positioned (via `transform:
translateY(...)` or `top` offsets) to land exactly where they'd
appear if the full list were actually rendered. Scroll further, and
the previously-rendered DOM nodes are recycled — their content swapped
for the newly-visible items' data — rather than the DOM growing
unbounded. This is why virtualization requires either a fixed item
height or an estimation/measurement strategy: computing "which indices
are visible at this scroll offset" requires knowing (or estimating)
how tall each item is before it's ever rendered.
