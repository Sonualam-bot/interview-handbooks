...
### The Bailout Happens BEFORE the Function Runs — The Opposite of useState's Bailout

This is worth contrasting directly with the `useState` same-value
bailout (Chapter 33): when you call `setCount` with an unchanged
value, the component function still executes once before React can
decide nothing meaningfully changed. `React.memo` is the opposite —
its shallow prop comparison happens during reconciliation, *before*
React ever calls the component function for this render. If every
prop is `Object.is`-equal to last render's and the fiber itself has no
independently pending state/context update, React skips calling the
function entirely and reuses the previous render's output wholesale.
This is a genuinely stronger bailout than `useState`'s — no component
code runs at all — which is also exactly why a `React.memo`'d
component's own internal `console.log` won't appear when a bailout
occurs, while a component doing a same-value `setState` bailout will
still log once.

### Why the Comparison Is Shallow, One Level, Never Recursive

`React.memo`'s default comparator checks each prop key with `Object.is`
— not a deep equality check. This is a deliberate cost trade-off, the
same one behind dependency array comparisons (see `useEffect`): a deep
comparison would need to walk arbitrarily nested structures on every
single render of every memoized component, potentially costing more
than the render it's trying to avoid, especially for large or
cyclical prop values. A shallow check is O(number of props) — cheap
and predictable regardless of what's nested inside those props. The
cost of that cheapness is exactly what section 5 of this chapter
about Object Props describes: a new object literal every render
defeats the shallow check even when its contents are identical,
because shallow comparison was never designed to look inside.
