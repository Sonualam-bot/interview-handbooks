...
### Why `React.memo` Can't Block Context Updates — The Actual Mechanism

Section 8 later in this chapter states the fact; here's the mechanism
behind it. `React.memo`'s bailout works by comparing a fiber's *props*
against last render's props during reconciliation (see `React.memo`
chapter) — it's a props-only check. Context doesn't travel through
props at all: when a Provider's value changes, React runs a separate
context-propagation walk that finds every fiber subscribed to that
context (each consumer's fiber keeps a dependency list recording which
contexts it reads) and marks those fibers as needing to update
directly — bypassing the normal parent-to-child props/reconciliation
path entirely. A memoized component sitting between the Provider and a
context-consuming descendant never even gets asked "did your props
change" for this purpose; the propagation walk finds the consumer
fiber and flags it regardless of what any intermediate fiber's memo
comparison would have concluded. This is why "wrap it in `React.memo`"
is not a valid fix for excessive context-driven re-renders — the two
mechanisms operate on entirely different paths through the tree.

### Why "Nearest Provider Wins" Is a Consequence of the Same Propagation Walk

The propagation walk doesn't broadcast one global value — it walks
down the tree carrying whatever the *nearest* active Provider's value
is at each point, so a consumer nested inside two nested
`ThemeContext.Provider`s reads the inner one's value simply because
that's the value in scope at the point in the tree where the walk
reaches that consumer. This isn't a special-cased rule; it falls out
naturally from context propagation being a tree walk rather than a
flat broadcast to every subscriber of "the" Provider.
