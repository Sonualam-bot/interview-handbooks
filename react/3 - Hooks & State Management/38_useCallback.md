...
### `useCallback(fn, deps)` Is Actually `useMemo(() => fn, deps)`

This isn't an analogy — it's literally how `useCallback` is
implemented. `useMemo` memoizes whatever value its factory function
returns; `useCallback` just calls `useMemo` internally with a factory
that returns the function itself, unchanged, instead of computing
anything. Both share the exact same storage slot mechanism (a
`[value, deps]` tuple on the fiber's Hook list, compared with
`Object.is` per index — see `useMemo`), and both skip their "work" —
`useMemo`'s calculation, `useCallback`'s function recreation — under
the identical condition: unchanged dependencies. Every rule that
applies to `useMemo`'s cache (single slot, no history, a performance
hint rather than a correctness guarantee) applies to `useCallback` for
exactly the same underlying reason.

### Why Wrapping a Function Alone Never Prevents Anything

`useCallback` only stabilizes the *function's identity* — it does
nothing to the component that receives the function as a prop. If
that child isn't wrapped in `React.memo`, the child re-renders whenever
its parent does regardless of whether the function prop changed,
because plain (non-memoized) components have no comparison step at
all — they always re-render when their parent does. This is why
`useCallback` is frequently described as "useless without
`React.memo`": the memoized function is a necessary condition for
skipping the child's re-render, but on its own it isn't a sufficient
one — nothing downstream is checking the identity `useCallback`
preserved unless something (`React.memo`, or a dependency array
somewhere) actually reads it.
