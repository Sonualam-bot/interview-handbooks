...
### Why `ref` Was Never Just Another Prop, Mechanically

When React creates an element from JSX, it pulls `ref` (and `key`) out
of the attributes *before* constructing the `props` object — both are
stored as separate fields on the element/fiber, never inside `props`
itself (see React Elements, Handbook 1). This is deliberate: `key` and
`ref` are instructions to *React's own bookkeeping* (identity for
reconciliation, and a handle into the rendered result), not data meant
for the component to consume like any other prop. That's the literal
reason `<MyInput ref={x} />` couldn't just work like any other prop
being destructured inside `MyInput(props)` before React 19's ref
model — `ref` was never placed in `props` for a function component to
find there in the first place. `forwardRef` exists specifically to
hand that separately-extracted value back to the function as an
explicit second argument, since there was no other channel for it to
arrive through.

### Why Function Components Specifically Needed This (Classes Never Did)

A class component instance is itself a real, addressable object — when
you write `<ClassInput ref={x} />`, React can set `x.current` to point
directly at that instance, because the instance already exists as
something to point to. A function component has no such persistent
instance — it's just a function that gets called and returns a value;
there is nothing analogous to "the instance" for a ref to attach to by
default. `forwardRef` (and later, the React 19 ref-as-prop model)
exists to give function components an explicit way to say what a ref
pointed at them *should* resolve to (usually a DOM node they render,
via `useImperativeHandle` — see that chapter), because unlike classes,
there's no implicit answer.
