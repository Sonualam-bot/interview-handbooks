...
### A Custom Hook Isn't a Separate Runtime "Thing" — It's Not Even Special to React

This is the fact that makes every other rule in this chapter fall out
naturally: `useCounter` is not a distinct kind of object React tracks
separately. It's a plain JavaScript function, and when `Counter` calls
it, the `useState` call *inside* `useCounter` still adds its node to
**`Counter`'s fiber's Hook list** — at whatever position it falls in
the sequence of Hook calls `Counter` makes, directly or indirectly.
React has no concept of "this state belongs to the `useCounter`
custom Hook" — it only ever sees "this is the 2nd Hook call made
while rendering this fiber." The custom Hook is purely a source-code
organizing device; at runtime, calling a custom Hook is
indistinguishable from inlining its body directly into the caller.

### Why This Single Fact Explains Two Separate Rules You'd Otherwise Have to Memorize

Because a custom Hook's internal Hook calls are really just more
entries in the *calling* component's Hook list:

-   **Two components calling the same custom Hook never share
    state** — `Counter A` and `Counter B` each have their own fiber,
    so each gets its own independent Hook list; `useCounter`'s
    `useState` call adds a separate node to each. There is no shared
    storage anywhere for the custom Hook to "own."
-   **Custom Hooks follow the exact same Rules of Hooks as built-in
    ones, with zero special-casing** — because there's no separate
    mechanism to special-case. A conditional `useState` call inside a
    custom Hook desynchronizes the caller's Hook list pointer walk
    exactly as if that `useState` had been written directly inside
    the component (see Rules of Hooks) — the custom Hook boundary is
    invisible to the ordering mechanism entirely.
