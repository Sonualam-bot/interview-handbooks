...
### Why "After Commit" Specifically Means "After Paint," Mechanically

Passive effects (`useEffect`) aren't run as the last step of commit —
they're scheduled as a *separate* unit of work, handed to the same
Scheduler that handles rendering (see Handbook 2). Concretely: commit
finishes, the browser is free to paint, and only after that does
React's scheduler run the queued effect callbacks — deliberately
giving the browser a chance to paint the just-committed DOM before
your effect code executes. This is why `useEffect` never blocks the
user from seeing a visual update, and it's the precise contrast with
`useLayoutEffect` (Chapter 43), which runs synchronously inside commit
itself, before paint.

### Why an Effect's Cleanup Always Sees the Props/State From When It Was Created

Every render that runs a component creates a brand-new effect
callback (and if returned, a brand-new cleanup function) — both are
ordinary closures over that render's props/state, exactly like every
other function created during render. React stores this render's
effect object in the fiber's effect list, and when a *later* render's
version of the effect needs to run, React first invokes the cleanup
function that was captured from the *previous* render, not the
current one. This is why a WebSocket's cleanup always disconnects the
room ID it originally connected to — even if props have since changed
— because the cleanup closure was frozen at the moment its effect was
created, with no way to observe values from a render that happened
after it.
