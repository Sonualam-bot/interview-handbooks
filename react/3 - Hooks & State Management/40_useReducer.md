...
### `dispatch` Uses the Exact Same Update Queue as `setState` — Here's Why That Matters

Every fiber with reducer or state Hooks has a per-Hook pending-update
queue (a linked list — see Handbook 2, Batching). Calling `dispatch(action)`
appends an update object carrying that action to the queue, exactly
like calling a state setter appends an update carrying a value or
updater function. The difference is what React does when it processes
the queue: for `useState`, each queued entry is either applied
directly or (for a functional updater) called with the accumulated
state; for `useReducer`, *every* queued action is replayed through the
same reducer function, in order, threading the result of each call
into the next. This is the concrete reason `useReducer` never has the
"stale closure" ambiguity that plain `setCount(count + 1)` has when
called multiple times in one handler: the reducer function computes
each next state from whatever the *previous queued action* produced,
not from a `count` value captured in the calling closure.

### Why This Makes useReducer's Sequencing Guarantees Different From useState's
...
with `reducer = (state, action) => action.type === "increment" ? state + 1 : state`
always produces `+3`, unconditionally — because each dispatched action
is replayed against the *reducer function*, which always reads the
latest accumulated state as its first argument, never a value frozen
in an event-handler closure. Compare this to `setCount(count + 1)`
called three times, which produces `+1` (Chapter 33) because `count`
there *is* frozen in the closure. `useReducer` sidesteps that failure
mode structurally, by moving the "how do I compute next state" logic
out of the closure and into a function whose input is always supplied
fresh by the queue-processing step itself.
